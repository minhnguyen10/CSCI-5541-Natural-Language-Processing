...
def calculate_prob(n_model,n1_model, train_set, dev_set):
    vocab = set(words.words())
    ...
    for sentence in dev_set:
        ...
        for i in range(len(toks) - n + 1):
            if n==1:
                ...
                nominator = smooth(nominator, k = 6, model= n_model)
            else:
...
# Predict
def predict(n_models, n1_models, train_set, dev_set):
    results = []
    for i in range(len(n_models)):
        probs = calculate_prob(n_models[i], n1_models[i], train_set, dev_set)
...
def main():
    ...
    # print(args.test)
        ...
        for i in range(len(dev_sets)):
            ...
            dev_probs = predict(n_models,n1_models, dev_sets[i], train_sets[i])
            print(dev_probs)
